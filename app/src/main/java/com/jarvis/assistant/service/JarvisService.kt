...
import com.jarvis.assistant.ai.GeminiApiEngine
...
import com.jarvis.assistant.core.NetworkMonitor
...
class JarvisService : Service() {
    ...
    private lateinit var geminiEngine: GeminiApiEngine
    ...
    private fun beginIdleListening() {
        ...
        updateNotification("Listening for \"Friday\"…")
        ...
        scope.launch {
            val reply = runCatching {
                when (val result = router.route(text)) {
                    ...
                    is CommandRouter.RouteResult.NeedsLlm -> generateReply(result.prompt)
                }
    ...
    private fun speak(text: String) {
        JarvisState.pushTranscript("Friday: $text")
    ...
    /**
     * Auto-switches per request: uses the cloud Gemini API when the device
     * is online AND a key is configured (smarter, more current answers), and
     * silently falls back to the fully offline MediaPipe model otherwise —
     * either because there's no connectivity, no key, or the API call fails.
     */
    private suspend fun generateReply(prompt: String): String {
        val online = ApiConfig.isConfigured() && NetworkMonitor.isOnline(this)
        if (online) {
            val cloudResult = runCatching { geminiEngine.generate(prompt) }
            if (cloudResult.isSuccess) return cloudResult.getOrThrow()
            // Cloud call failed (timeout, quota, bad key, etc.) — fall through to offline.
        }
